...
using namespace thur;

ThurSerialPort::ThurSerialPort()
    : serial_port_(-1)
    , rx_frame_length_(0)
...
return_type ThurSerialPort::open(const std::string & port_name)
{
    serial_port_ = ::open(port_name.c_str(), O_RDWR | O_NOCTTY);
    RCLCPP_ERROR(rclcpp::get_logger("ThurSerialPort"), "Serial_port_: %i", serial_port_);
    //RCLCPP_ERROR(rclcpp::get_logger("ThurSerialPort"), "Port_name: %c", port_name);

    if (serial_port_ < 0) {
        RCLCPP_ERROR(rclcpp::get_logger("ThurSerialPort"), "Failed to open serial port: %s (%d)", strerror(errno), errno);
        return return_type::ERROR;
    }

    struct termios tty_config{};
    if (::tcgetattr(serial_port_, &tty_config) != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("ThurSerialPort"), "Failed to get serial port configuration: %s (%d)", strerror(errno), errno);
        close();
        return return_type::ERROR;
    }
    ...
    */
    ...
    if (::tcsetattr(serial_port_, TCSANOW, &tty_config) != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("ThurSerialPort"), "Failed to set serial port configuration: %s (%d)", strerror(errno), errno);
        close();
        return return_type::ERROR;
    }
...
return_type ThurSerialPort::close()
{
    if (is_open()) {
        ::close(serial_port_);
        serial_port_ = -1;
...
return_type ThurSerialPort::read_frames(std::vector<SerialHdlcFrame>& frames)
{
    // Read data from the serial port
    const ssize_t num_bytes = ::read(serial_port_, rx_buffer_, 256);
    if (num_bytes == -1) {
        RCLCPP_ERROR(rclcpp::get_logger("ThurSerialPort"), "Failed to read serial port data: %s (%d)", strerror(errno), errno);
        return return_type::ERROR;
    }
...
return_type ThurSerialPort::write_frame(const uint8_t* data, size_t size)
{
    if (!is_open()) {
        return return_type::ERROR;
    }
    ...
    if (::write(serial_port_, tx_frame_buffer_, tx_frame_length_) == -1) {
        RCLCPP_ERROR(rclcpp::get_logger("ThurSerialPort"), "Failed to write serial port data: %s (%d)", strerror(errno), errno);
        return return_type::ERROR;
    }
...
bool ThurSerialPort::is_open() const
{
    return serial_port_ >= 0;
}

void ThurSerialPort::encode_byte(uint8_t data)
{
...
void ThurSerialPort::decode_byte(uint8_t data, std::vector<SerialHdlcFrame>& frames)
{
    ...
    if (rx_frame_length_ == THUR_SERIAL_SERIAL_FRAME_MAX_SIZE) {
        rx_frame_length_ = 0;
        rx_frame_crc_ = HDLC_CRC_INIT_VALUE;
    }
}

uint16_t ThurSerialPort::crc_update(uint16_t crc, uint8_t data)
{
...
char* to_string() {
    std::string ret = "Name of serial port '%s'", serial_port_;
    char* ter = new char [ret.length() + 1];
    std::strcpy(ter, ret.c_str());
    return ter;
}